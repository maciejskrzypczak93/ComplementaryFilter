...
public class MainActivity extends AppCompatActivity {
    ...
    private static float[] logMap(float[] quat){
        ...
        if(qvNorm > 1e-6){
            res[0] = quat[1]/(float)qvNorm;
            res[1] = quat[2]/(float)qvNorm;
            res[2] = quat[3]/(float)qvNorm;
        }
        else{
            // 1/sqrt(3), so norm = 1
    ...
    //////////////////////////////////
        ...
        /*for(int i=0;i<tSize;i++) {
            ...
            if(t_gyro_int.contains(i)){
                ...
                SensorManager.getRotationMatrixFromVector(deltaMat, quat2rotVec(quaternion_gyro.get(i)));
        ...
        for(int i=0;i<tSize;i++) {
            ...
            if(t_gyro_int.contains(i)){
                if(init){
                    filter_init[0] = quaternion_orient_logmap.get(i)[0];
                    filter_init[1] = quaternion_orient_logmap.get(i)[1];
                    filter_init[2] = quaternion_orient_logmap.get(i)[2];
                    filter_init_exp = expMap(filter_init);
                    quaternion_gyro_mul1.add(quatMul(filter_init_exp, quaternion_gyro.get(licznik_gyro)));
                    quaternion_gyro_mul.add(quaternion_gyro_mul1.get(i).clone());
                    init = false;
                }
                else{
                    quaternion_gyro_mul1.add(quatMul(quaternion_exp.get(i-1), quaternion_gyro.get(licznik_gyro)));
                    quaternion_gyro_mul.add(quaternion_gyro_mul1.get(i).clone());
                }
    ...
    matrixQuat[3] = qz;
        ...
        for(int i=0; i<quaternion_exp.size(); i++){
            QxR1.add(quatMul(quaternion_exp.get(i), matrixQuat));
    ...
    public void buttonOnClick(View v){
        ...
        if(t_orient_int.contains(licznik_przycisk)) {
            ...
            if (licznik_przycisk==10){
                ...
                //kwaternion=Euler2Quat(0.9356f, -0.00690f, 1.07062f  );
                kwaternion=Euler2Quat(-0.2962f, -1.39009f, 1.0010f  );//test - wynik z wyjscia filtra na kwaternion, wartosci wyliczone z matlaba
                licznik_przycisk=10;
            }
        }
        else{
